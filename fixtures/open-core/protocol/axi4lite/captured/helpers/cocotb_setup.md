# cocotb setup for AXI4-Lite captured fixtures

The AXI4-Lite captured fixtures in this directory are produced by running
...
2. **Patch `iverilog_dump.v` to add an `aresetn` net** (see "Why we patch
   ...
   `helpers/forencich_axil_ram_aresetn_dump.patch` and applied with `patch -p1`:

   ```bash
   cd /tmp/wcrux-captures/verilog-axi/tb/axil_ram
   PATH=~/.venvs/wavecrux-cocotb/bin:$PATH make WAVES=1 SIM=icarus  # first run autogenerates iverilog_dump.v
   patch -p1 < /path/to/this/helpers/forencich_axil_ram_aresetn_dump.patch
   ```

3. **Re-run the testbench with `WAVES=1`** so iverilog re-compiles with the
   patched dump file:
   ...
   This produces `axil_ram.fst` in the testbench directory.

4. **Copy the FST into both fixture trees** (`test/` and `verification/`),
   renaming to a `forencich_<scenario>.fst` slug:

   ```bash
   cp /tmp/wcrux-captures/verilog-axi/tb/axil_ram/axil_ram.fst \
      <repo>/test/fixtures/protocol/axi4lite/captured/forencich_axil_ram.fst
   cp /tmp/wcrux-captures/verilog-axi/tb/axil_ram/axil_ram.fst \
      <repo>/verification/fixtures/protocol/axi4lite/captured/forencich_axil_ram.fst
   ```

5. **Write the `<name>.fixture.json` sidecar** with the per-fixture signal
   bindings. See `forencich_axil_ram.fixture.json` for the canonical
   pattern — every required AXI4-Lite signal name maps to a
...
6. **Regenerate the snapshot:**
   ...
   REGENERATE=1 flutter test \
     test/services/decoders/axi4lite_captured_fixtures_test.dart
   ```

   This populates `<name>.expected_transactions.json`. Mirror the snapshot
   into `verification/fixtures/protocol/axi4lite/captured/` too.
...
The open-core `Axi4LiteDecoder` requires an `aresetn` signal binding
(active-low synchronous reset, per the AXI spec). Forencich's testbenches
drive an active-**high** `rst` signal, so a direct binding would have
inverted semantics — the decoder would skip every clock cycle when the bus
is live and process every cycle when the bus is in reset.
...
a single `wire aresetn = !axil_ram.rst;` declaration plus a
...
The patch lives at `helpers/forencich_axil_ram_aresetn_dump.patch` and is
committed alongside this guide so future contributors can reproduce the
capture exactly.
...
fixture-triplet layout are identical — only the testbench target and the
binding shape (no burst signals: no `awlen` / `awsize` / `awburst` /
`awid` / `wlast` / `bid` / `arlen` / `arsize` / `arburst` / `arid` /
`rid` / `rlast`) differ. New AXI4-Lite captures from other upstream
projects (LiteX `litex/soc/cores/lite_axi`, OpenTitan `tlul_axi`,
zipcpu/wb2axip's `axilite*` modules) can land in the same `captured/`
dir alongside `forencich_axil_ram.fst`.
